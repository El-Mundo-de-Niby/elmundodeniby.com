...
const Chatbot = ({ navigateTo, setIsChatbotOpen }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (result.candidates && result.candidates.length > 0 &&
        ...
        setMessages((prevMessages) => [...prevMessages, { sender: 'ai', text: 'Sorry, I couldn\'t generate a response. Please try again.' }]);
      }
    } catch (error) {
  ...
  return (
    // Pass h-full to FadeInOnMount so it takes 100% height of its parent (the fixed wrapper)
    <FadeInOnMount duration={300} className="h-full">
      <div className="flex flex-col h-full bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden">
        <div className="p-4 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-bold text-lg border-b border-gray-200 dark:border-gray-600">
          AI Support
          <p className="text-sm font-normal text-gray-600 dark:text-gray-300 mt-1">Ask our AI or contact us</p>
        </div>
        ...
        <div className="p-4 pt-0 bg-gray-100 dark:bg-gray-700 flex flex-col items-center">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 text-center">
            Couldn't find what you were looking for with AI?
          </p>
          ...
          >
            <Mail size={20} className="mr-2" /> Contact by Email
          </button>
          ...
          <a
            ...
            className="w-full inline-flex items-center justify-center bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-100 px-4 py-3 rounded-full text-md font-semibold hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors shadow-md hover:scale-[1.02] duration-200 transform"
          >
